...
class SSIM(torch.nn.Module):
    ...
    def forward(self, img1, img2):
        ...
        return ssim(img1/img1.max(), img2/img2.max(), window=window.cuda(), window_size=self.window_size, size_average=self.size_average)
...
class PearsonCorrelation(nn.Module):
    ...
    def forward(self, x, y):
        ...
        x_norm = (x - x_mean)
        y_norm = (y - y_mean)
        r_num = torch.sum(x_norm * y_norm)
        r_den = torch.sqrt(torch.sum(x_norm**2)) * torch.sqrt(torch.sum(y_norm**2))
        correlation = torch.mean(x_norm * y_norm)
        return correlation
...
class BlurryLossMSE(nn.Module):
    ...
    def forward(self, x, y, PSF):
        imgD = F.conv2d(x, PSF, padding=PSF.shape[2] // 2)
        imgB = y
        #imgD = (imgD-imgD.min()) / (imgD.max()-imgD.min())
